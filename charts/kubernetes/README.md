...
| scraper.defaultTransform.changes.exclude[1] | string | `"change_type == \"diff\" && config_type == \"Kubernetes::Canary\" && summary.startsWith(\"status.\")"` |  |
...
| scraper.defaultTransform.changes.mapping[1].filter | string | `"change.change_type == 'diff' && jq('.status.containerStatuses[0].restartCount', patch) != null && jq('.status.containerStatuses[0].restartCount', patch) > 0\n"` |  |
| scraper.defaultTransform.changes.mapping[1].severity | string | `"high"` |  |
| scraper.defaultTransform.changes.mapping[1].type | string | `"PodCrashLooping"` |  |
| scraper.defaultTransform.exclude | list | `[]` |  |
| scraper.defaultTransform.mask | list | `[]` |  |
...
| scraper.name | string | `"{{ .Values.clusterName }}"` |  |
